#!/usr/bin/env python3
"""
PointTransformerV3 Evaluation Script

Evaluation script for PTv3 point cloud segmentation models.
Processes real-world scenes and generates segmentation results.

Usage:
    python PTv3/evaluate_ptv3.py --model-path path/to/model.pth --scene-dir path/to/scenes --output-dir results/
"""
...
class PTv3SceneProcessor:
    """Point cloud scene processor for PTv3 model inference"""
    
    def __init__(self, model, device, max_points_per_chunk=None, overlap_ratio=0.3, 
                 grid_size=0.02, adaptive_chunking=True):
        self.model = model
        self.device = device
    ...
    def adaptive_chunk_size_calculation(self, points, target_chunk_density=15000):
        ...
        chunk_size = max(0.2, min(2.0, chunk_size))
    ...
    def prepare_ptv3_data_dict(self, chunk_points, chunk_colors):
        """Prepare chunk data for PTv3 model input"""
        ...
        if combined_feat.shape[1] != 3:
            if combined_feat.shape[1] > 3:
                combined_feat = combined_feat[:, :3]
            elif combined_feat.shape[1] < 3:
                pad_amount = 3 - combined_feat.shape[1]
                padding = np.zeros((combined_feat.shape[0], pad_amount), dtype=np.float32)
                combined_feat = np.concatenate([combined_feat, padding], axis=1)
        
        batch = np.zeros(len(coord), dtype=np.int64)
        
        coord_tensor = torch.tensor(coord, dtype=torch.float32, device=self.device)
        feat_tensor = torch.tensor(combined_feat, dtype=torch.float32, device=self.device)
        batch_tensor = torch.tensor(batch, dtype=torch.long, device=self.device)
    ...
    def run_inference_on_chunk(self, chunk_points, chunk_colors):
        """Run PTv3 model inference on a single chunk"""
        result = self.prepare_ptv3_data_dict(chunk_points, chunk_colors)
    ...
    def create_segmentation_images(self, points, colors, predictions, confidences, scene_name, save_dir):
        ...
        fig.suptitle(f'PTv3 Segmentation Results - {scene_name}', fontsize=16, fontweight='bold')
    ...
    def create_batch_summary(self, all_results, output_dir):
        ...
        fig.suptitle('PTv3 Batch Processing Summary', fontsize=16, fontweight='bold')
...
class PTv3SegmentationModel(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if hasattr(point, 'feat'):
            feat = point.feat
        else:
            feat = point['feat'] if isinstance(point, dict) else point
...
def create_model(device, model_path):
    ...
    try:
        ...
        print("Successfully imported PointTransformerV3")
    except ImportError as e:
        print(f"Error importing PointTransformerV3: {e}")
        return None
    ...
    try:
        ptv3_backbone = PTv3M1(**ptv3_config).to(device)
        print("PointTransformerV3 backbone initialized")
    except Exception as e:
        print(f"Error initializing PTv3: {e}")
        return None
    
    # Get output channels
    test_data = {
        'coord': torch.randn(1000, 3, device=device),
        'feat': torch.randn(1000, feat_dim, device=device),
        'batch': torch.zeros(1000, dtype=torch.long, device=device),
        'offset': torch.tensor([1000], dtype=torch.long, device=device),
        'grid_size': grid_size
    }
    
    with torch.no_grad():
        test_output = ptv3_backbone(test_data)
        if hasattr(test_output, 'feat'):
            output_channels = test_output.feat.shape[-1]
        else:
            output_channels = test_output.shape[-1]
    
    # Create full model
    seg_head = SegmentationHead(output_channels, num_classes).to(device)
    model = PTv3SegmentationModel(ptv3_backbone, seg_head).to(device)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate PTv3 point cloud segmentation model')
